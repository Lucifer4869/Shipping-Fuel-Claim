...
{
    [Required] public int ShipmentId { get; set; }//รหัสการเดินรถ
    [Required][Range(1, double.MaxValue)] public decimal ClaimAmount { get; set; }//จำนวนเงินที่ขอ
    [Required] public string Reason { get; set; } = string.Empty;//เหตุผล
    public string? ReceiptUrl { get; set; }//ใบเสร็จ
    [Required] public double MileageOut { get; set; }//เลขไมล์ออก
    [Required] public double MileageIn { get; set; }//เลขไมล์เข้า
}
...
{
    public int Id { get; set; }//รหัสใบขอเติมน้ำมัน
    ...
    public string DriverName { get; set; } = string.Empty;//ชื่อผู้ขับขี่
    public decimal ClaimAmount { get; set; }//จำนวนเงินที่ขอ
    public string Reason { get; set; } = string.Empty;//เหตุผล
    public string? ReceiptUrl { get; set; }//ใบเสร็จ
    public double MileageOut { get; set; }//เลขไมล์ออก
    public double MileageIn { get; set; }//เลขไมล์เข้า
    public double Distance => MileageIn - MileageOut;//ระยะทาง
    public string Status { get; set; } = string.Empty;//สถานะ
...
{
    public int Id { get; set; }//รหัสการตรวจสอบย้อนหลัง
    public string TableName { get; set; } = string.Empty;//ตาราง
    public int RecordId { get; set; }//รหัสที่ถูกตรวจสอบ
    public string? ReferenceNumber { get; set; } // เช่น FLC-2024-001
    public string Action { get; set; } = string.Empty;//การกระทำ
    public string? OldValue { get; set; }//ค่าเก่า
    public string? NewValue { get; set; }//ค่าใหม่
    public string PerformedByName { get; set; } = string.Empty;//ชื่อผู้ตรวจสอบ
    public string? PerformedByRole { get; set; }//บทบาทผู้ตรวจสอบ
    public DateTime CreatedAt { get; set; }//เวลาที่สร้าง
}
...
public class CreateUserRequest
{
    ...
    [Required] public string Password { get; set; } = string.Empty;//รหัสผ่าน
    [Required] public string FullName { get; set; } = string.Empty;//ชื่อ
    [Required] public string Role { get; set; } = string.Empty;//บทบาท
    public string VehiclePlate { get; set; } = string.Empty;//ทะเบียนรถ
}

public class UpdateUserRequest
{
    public string? FullName { get; set; }//ชื่อ
    public string? Email { get; set; }//อีเมล
    public string? Role { get; set; }//บทบาท
    public string? Password { get; set; }//รหัสผ่าน
    public string? VehiclePlate { get; set; }//ทะเบียนรถ
    public bool? IsActive { get; set; }//สถานะ
}

public class UserDto
{
    public int Id { get; set; }//รหัสผู้ใช้
    public string Username { get; set; } = string.Empty;//ชื่อผู้ใช้
    public string Email { get; set; } = string.Empty;//อีเมล
    public string FullName { get; set; } = string.Empty;//ชื่อ
    public string Role { get; set; } = string.Empty;//บทบาท
    public string VehiclePlate { get; set; } = string.Empty;//ทะเบียนรถ
    public bool IsActive { get; set; }//สถานะ
    public DateTime CreatedAt { get; set; }//เวลาที่สร้าง
